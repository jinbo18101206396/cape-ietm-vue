<template>
  <div>
    <a-form-model ref="form" :model="model" :rules="validatorRules" class="password-retrieval-form" @keyup.enter.native="nextStep">
      <a-form-model-item label="内网邮箱" required prop="phone" :labelCol="{span: 5}" :wrapperCol="{span: 19}">
        <a-input v-model="model.phone" type="text" autocomplete="false" placeholder="请输入内网邮箱地址">
      ...
      <a-form-model-item v-if="show" required prop="captcha" label="验证码" :labelCol="{span: 5}" :wrapperCol="{span: 19}">
        <a-row :gutter="16">
          <a-col class="gutter-row" :span="12">
            <a-input @change="captchaChange" v-model="model.captcha" type="text" placeholder="内网邮箱短信验证码">
  ...
  export default {
    ...
    data() {
      return {
        ...
        validatorRules: {
          phone: [
            { required: true, message: '请输入内网邮箱号码!' },
