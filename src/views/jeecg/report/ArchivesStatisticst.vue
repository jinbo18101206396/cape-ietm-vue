        ...
        <a-row :gutter="24">
          <a-col :span="10">
            <a-radio-group :value="pieType" @change="statisticst">
              <a-radio-button value="project">按项目统计</a-radio-button>
              <a-radio-button value="year">按年统计</a-radio-button>
              <a-radio-button value="month">按月统计</a-radio-button>
              <a-radio-button value="category">按类别统计</a-radio-button>
              <a-radio-button value="cabinet">按柜号统计</a-radio-button>
...
export default {
  ...
  data() {
    return {
      ...
      tabStatus: "bar",
      url: {
        getProjectCountInfo: "/hrutilization/getProjectCountInfo",
  ...
  methods: {
    loadDate(url, type, param) {
      getAction(url, param, 'get').then((res) => {
        if (res.success) {
          this.countSource = [];
          if (type === 'project') {
            this.getProjectCountSource(res.result);
          }
          if (type === 'year') {
    ...
    getUrl(type, param) {
      let url = "";
      if (type === 'project') {
        url = this.url.getProjectCountInfo;
      }
      if (type === 'year') {
