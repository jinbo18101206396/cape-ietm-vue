<template>
  <div class="j-statistical-query-box">
    <slot
      name="button"
      :isActive="statisticalQueryFlag"
      :isMobile="izMobile"
      :open="handleOpen"
      :reset="handleReset"
    >
      <a-tooltip v-if="statisticalQueryFlag" v-bind="tooltipProps" :mouseLeaveDelay="0.2">
        <!-- begin 不知道为什么不加上这段代码就无法生效 -->
        <span v-show="false">{{ tooltipProps }}</span>
        <!-- end 不知道为什么不加上这段代码就无法生效 -->
        <!-- <template slot="title">
          <span>已有月报统计查询条件生效</span>
      ...
      <a-button v-else type="primary" icon="filter" @click="handleOpen">月报统计查询</a-button>
    </slot>
    ...
    <j-modal
      ...
      :fullscreen="izMobile"
      draggable
      class="j-statistical-query-modal"
      ...
      <a-spin :spinning="loading">
        <a-row>
            ...
            <a-form v-else layout="inline">
              ...
              >
                <!-- <a-col :md="8" :xs="24" style="margin-bottom: 12px;">
                  <j-year-picker placeholder="请选择月报年度" v-model="item.year" style="width:100%" />
                </a-col>-->
                <a-col :md="8" :xs="24" style="margin-bottom: 12px;">
                  <a-month-picker v-model="item.month" placeholder="请选择年月" style="width:100%" />
                </a-col>
                <a-col :md="8" :xs="24" style="margin-bottom: 12px;">
                  <a-tree-select
                    showSearch
                ...
                <a-col :md="4" :xs="0" style="margin-bottom: 12px;">
                  <a-tooltip title="增加查询条件">
                    <a-button @click="handleAdd" icon="plus"></a-button>
                  </a-tooltip>&nbsp;
                  <a-tooltip title="删除当条查询条件">
          ...
          <a-col :sm="24" :md="5">
            ...
            <a-card class="j-statistical-query-history-card" :bordered="true">
              <div slot="title">保存的查询</div>

              <a-empty
                v-if="saveTreeData.length === 0"
                class="j-statistical-query-history-empty"
                description="没有保存任何查询"
              />
              <a-tree
                v-else
                class="j-statistical-query-history-tree"
                showIcon
...
import JYearPicker from './JYearPicker.vue'

export default {
  name: 'JStatisticalQuery',
  mixins: [mixinDevice],
  components: { JAreaLinkage, JMultiSelectTag, JDate, JSelectDepart, JSelectMultiUser, JYearPicker },
  props: {
    /*
     fieldList: [{
        value:'',
    ...
    * */
    callback: {
      type: String,
      required: false,
      default: 'handleStatisticalQuery'
    },
  ...
  data() {
    return {
      ...
      saveTreeData: [],
      // 保存查询条件的前缀名
      saveCodeBefore: 'JStatisticalQuerySaved_',
      // 查询类型，过滤条件匹配（and、or）
      matchType: 'and',
      statisticalQueryFlag: false,
    }
  },
  computed: {
    izMobile() {
        ...
        val.forEach(item => {
          ...
          if (hasChildren) {
            data.children = data.children.map(item2 => {
              ...
              // update--begin--author:sunjianlei-----date:20220121------for：【JTC-1167】【表单设计器】月报统计查询，一对一字段查询不好使
              // 是否仅包含字段名，不需要拼接子表表名
              if (!data.onlyFieldName) {
                child.value = data.value + ',' + child.value
              }
              // update--end--author:sunjianlei-----date:20220121------for：【JTC-1167】【表单设计器】月报统计查询，一对一字段查询不好使
              child.val = ''
              return child
            })
            data.val = ''
  ...
  methods: {
    ...
    getDictInfo(item) {
      ...
      console.log('月报统计查询字典信息', str)
      return str
    },
    handleOk() {    
      this.queryParamsModel[0].val = this.queryParamsModel[0].field;      
    ...
    emitCallback(event = {}, loadStatus = true) {
      ...
      // this.statisticalQueryFlag = (params && params.length > 0)
      for (let param of params) {
        if (Array.isArray(param.val)) {
          param.val = param.val.join(',')
        }
      }
      console.debug('---月报统计查询参数--->', { params, matchType })    
    ...
    handleSelected(node, item) {
      ...
      //update-begin-author:taoyan for: VUEN-242【online表单 月报统计查询】开关组件设置扩展参数为[0,1] 时，月报统计查询选择后查询仍然是Y/N
      ...
      for (let i = 0; i < array.length; i++) {
        ...
        if (item == null || Object.keys(item).length <= 0) {
          ...
          if (Array.isArray(item.options)) {
            ...
            //update-begin-author:taoyan date:20200819 for:【开源问题】 月报统计查询 下拉框作为并且选项很多多多 LOWCOD-779
            delete item.options
            //update-end-author:taoyan date:20200819 for:【开源问题】 月报统计查询 下拉框作为并且选项很多多多 LOWCOD-779
          }
        }
      }
      return array
...
.j-statistical-query-box {
  display: inline-block;
}

.j-statistical-query-modal {
  .j-statistical-query-history-card {
    /deep/ .ant-card-body,
  ...
  .j-statistical-query-history-empty {
  ...
  .j-statistical-query-history-tree {
