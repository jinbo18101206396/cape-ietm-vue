...
(function( $ ) {
	...
	Formatter.prototype = {
		...
		xml: function(text) {
			var ar = text.trim().replace(/>\s{0,}</g,"><")
						 .replace(/</g,"~::~<")
				...
				str = '',
				ix = 0;
			//将\w 替换为 a-zA-Z0-9_\u4e00-\u9fa5 以支持中文节点20190705
			for(ix=0;ix<len;ix++) {
				...
				// <elm></elm> //
				if( /^<[a-zA-Z0-9_\u4e00-\u9fa5]/.exec(ar[ix-1]) && /^<\/[a-zA-Z0-9_\u4e00-\u9fa5]/.exec(ar[ix]) &&
					/^<[a-zA-Z0-9_\u4e00-\u9fa5:\-\.\,]+/.exec(ar[ix-1]) == /^<\/[a-zA-Z0-9_\u4e00-\u9fa5:\-\.\,]+/.exec(ar[ix])[0].replace('/','')) {
					str += ar[ix];
					if(!inComment) deep--;
				} else
				 // <elm> //
				if(ar[ix].search(/<[a-zA-Z0-9_\u4e00-\u9fa5]/) > -1 && ar[ix].search(/<\//) == -1 && ar[ix].search(/\/>/) == -1 ) {
					str = !inComment ? str += this.shift[deep++]+ar[ix].trim() : str += ar[ix];//增加了.trim() --20180411
				} else
				 // <elm>...</elm> //
				if(ar[ix].search(/<[a-zA-Z0-9_\u4e00-\u9fa5]/) > -1 && ar[ix].search(/<\//) > -1) {
				...
				if(ar[ix].search(/\/>/) > -1 ) {
					//str = !inComment ? str += this.shift[deep]+ar[ix] : str += ar[ix];
					//原来只有上面一行.需要将<elm/>改为<elem></elem>,改为以下.---in 20180108
					var ename='',str1='';
					if(ar[ix].indexOf(' ')==-1){
						ename=ar[ix].substr(1,ar[ix].length-3);
					} else {
						ename=ar[ix].substr(1,ar[ix].indexOf(' '));
					}
					str1=ar[ix].trim().replace(/\s+\/>/,'/>').replace("/>","></"+ename.trim()+">");//将/>前的空格去掉,将/>替换为></elem>
					str = !inComment ? str += this.shift[deep]+str1 : str += str1;
				} else
				// <? xml ... ?> //
				if(ar[ix].search(/<\?/) > -1) {
					str += this.shift[deep]+ar[ix]+(ar[ix].indexOf('<?xml')==0?'\n':'');//增加了+'\n'  --in 20181019
				} else
				// xmlns //
				if( ar[ix].search(/xmlns\:/) > -1  || ar[ix].search(/xmlns\=/) > -1) {
					//str += this.shift[deep]+ar[ix];//该行改为以下 --in 20180108
					str += " " + ar[ix];
				}
