// 重建Refs测试脚本 - 直接在编辑器页面的Console加载
// 用法：在DM编辑器页面Console中执行：
// var script = document.createElement('script'); script.src = '/test-regen.js'; document.head.appendChild(script);
...
(async function runRegenRefsTests() {
  ...
  try {
    ...
    try {
      ...
      if (errors.length === 0 && !hasOld && hasNew && hasDescription) {
        recordResult('TC-03', '已有refs块 - 应正确替换不误删后续内容', true, {
          旧refs已删除: !hasOld,
          新refs已添加: hasNew,
          description保留: hasDescription
        });
      } else {
